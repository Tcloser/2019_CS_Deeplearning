...
'''
argparse 
#命令行参数解析 python3 mainpro_FER.py -h 可以查看此文件的描述性
python3 mainpro_FER.py --model 'VGG19' 这种需要后面加上字符型输入
'''#已整理
...
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'#声明GPU序号
'''放置至GPU'''
...
if opt.model == 'VGG19':
    ...
    net = ResNet18()
'''
将之前训练所得到的参数加载 Public_model.t7
命令行 python3 mainpro_FER.py --resume
'''
if opt.resume:
    # Load checkpoint.
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        loss = criterion(outputs, targets)#输出的7种分类计算损失
        loss.backward()
