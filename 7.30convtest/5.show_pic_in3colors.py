...
def grey_(a):
    ...
    c = 255
    pic = a*1.
    size_x,size_y = np.shape(a)
    size_x = int(size_x)
    size_y = int(size_y)
    red = np.zeros((size_x,size_y))
    blue = np.zeros((size_x,size_y))
    green = np.zeros((size_x,size_y))
    picture = np.zeros((size_x,size_y,3))
    # print (a)
            ...
            if(pic[i][k] <= c / 4.):
                ...
                if(pic[i][k] <= c / 2.):
                    ...
                    if(pic[i][k] <= 3*c / 4.):
                        ...
                        green[i][k] = c
                        blue[i][k] = pic[i][k]
    # for i in range(size_x):#绿色为主
    #     for k in range(size_y):
    #         if(pic[i][k] <= c / 4.): #0-255/4
    #             red[i][k] = 0 #清除R
    #             green[i][k] = 4 * pic[i][k]
    #             blue[i][k] = c
    #         else:
    ...
    #                 red[i][k] = 0
    #                 green[i][k] = c
    #                 blue[i][k] = -4 * pic[i][k] + 2 * c
    #             else: 
    #                 if(pic[i][k] <= 3*c / 4.):
    #                     red[i][k] = 4 * pic[i][k] - 2 * c
    #                     green[i][k] = c
    #                     blue[i][k] = 0
    #                 else:
    #                     red[i][k] = c
    #                     green[i][k] = -4 * pic[i][k] + 4 * c
    #                     blue[i][k] = 0
    # # red.append(blue)
    # # red.append(green)
    picture[:,:,0] = blue
...
def main():
    ...
    # plt.imshow(image)
    '''cv2进图'''
    #image = cv2.imread('/home/dooncloud/桌面/practice/7.30convtest/1.jpg')# to ndarray
    image = cv2.imread('/home/dooncloud/桌面/practice/8.1smart_car/twonum_images/51.jpg')# to ndarray
    plt.subplot(3,3,1)
    plt.imshow(image)
    net = resnet(3,64,kernel_size=3,stride=2,padding=1)

    image = (cv2.resize(image,(224,224),interpolation=cv2.INTER_CUBIC) - 128.)/128. #线性差值 归一化 (48, 48, 3)
    print(image.shape)
    # 适用于CV2进图
    plt.subplot(3,3,2)
    plt.imshow(image)
